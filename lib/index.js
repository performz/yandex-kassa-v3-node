...
YandexKassa.PACKAGE_VERSION = require('../package.json').version;
YandexKassa.DEFAULT_BASE_HOST = 'https://payment.yandex.net';
YandexKassa.DEFAULT_BASE_PATH = '/api/v3/';
YandexKassa.DEFAULT_DEBUG = false;

// Use node's default timeout:
YandexKassa.DEFAULT_TIMEOUT = require('http').createServer().timeout;

function YandexKassa(shopId, secretKey) {
  var options = {};
  ...
  this.root = (options.base_host || YandexKassa.DEFAULT_BASE_HOST)
    + (options.base_path || YandexKassa.DEFAULT_BASE_PATH);
  this.debug = options.debug || YandexKassa.DEFAULT_DEBUG;
  this.timeout = options.timeout || YandexKassa.DEFAULT_TIMEOUT;
}

YandexKassa.prototype = {
  /**
   * Create new payment
...
function init(shopId, secretKey) {
  return new YandexKassa(shopId, secretKey);
}

module.exports = init;
module.exports.YandexKassa = YandexKassa;